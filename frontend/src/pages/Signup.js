...
const STATES = ['Punjab','Haryana','Uttar Pradesh','Maharashtra','Karnataka','Andhra Pradesh','Tamil Nadu','Rajasthan','Gujarat','Bihar','Madhya Pradesh','Odisha'];
    ...
    <div className="min-h-screen flex font-body">
      ...
      <div className="hidden lg:flex flex-1 flex-col justify-center px-12 bg-gradient-to-br from-agri-1 to-agri-3 relative overflow-hidden">
        ...
        <Link to="/" className="absolute top-8 left-8 flex items-center gap-2 text-white font-head text-xl font-bold">
          <div className="w-9 h-9 rounded-xl bg-white/20 flex items-center justify-center">🌿</div>AgriAssist
        </Link>
        <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.6 }} className="relative z-10 max-w-sm">
          ...
          <p className="text-white/65 leading-relaxed mb-8">Create your free account and start protecting your crops with the power of AI — no credit card required.</p>
      ...
      <div className="flex-1 flex items-center justify-center px-6 py-8 bg-white overflow-y-auto">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="w-full max-w-md">
          <Link to="/" className="lg:hidden flex items-center gap-2 font-head text-xl font-bold text-agri-1 mb-6">
            <div className="w-9 h-9 rounded-xl bg-agri-7 flex items-center justify-center">🌿</div>AgriAssist
          </Link>
          <h2 className="font-head text-2xl font-bold text-gray-900 mb-1">Create Your Account 🌱</h2>
          <p className="text-gray-400 text-sm mb-6">Free forever · No credit card needed</p>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <input value={form.name} onChange={e => setForm({...form, name: e.target.value})} required placeholder="Rajesh Singh"
            ...
            <div>
              ...
              <input type="email" value={form.email} onChange={e => setForm({...form, email: e.target.value})} required placeholder="farmer@example.com"
                className="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-agri-3 bg-gray-50 focus:bg-white transition-all" />
            </div>
            <div>
          ...
          <p className="text-center text-sm text-gray-400 mt-5">
            Already have an account? <Link to="/login" className="text-agri-3 font-semibold hover:underline">Login here</Link>
          </p>
        </motion.div>
      </div>
    </div>
