...
const NAV = [
  ...
  { path: '/calendar', icon: '📅', label: 'Crop Calendar' },
  { path: '/diseases', icon: '🦠', label: 'Disease Library' },
...
export default function AppLayout({ children, title, subtitle }) {
  ...
  const NavLink = ({ n, onClick }) => (
    ...
    >
      ...
      {n.badge && (
        <span className="text-xs font-bold bg-gradient-to-r from-agri-4 to-agri-2 text-white px-2 py-0.5 rounded-full">
          {n.badge}
        </span>
      )}
    </Link>
    ...
    <div className="flex h-screen overflow-hidden bg-agri-8 font-body">
      <aside className="hidden md:flex flex-col w-64 bg-white border-r border-agri-7 shrink-0">
        ...
        <nav className="flex-1 p-3 space-y-1 overflow-y-auto">
          {NAV.map(n => <NavLink key={n.path} n={n} />)}
        </nav>
        <div className="p-3 border-t border-agri-7">
          <div className="flex items-center gap-3 mb-3 p-2 rounded-xl bg-agri-8">
            {user?.picture ? (
              ...
              <div className="w-9 h-9 rounded-full bg-gradient-to-br from-agri-4 to-agri-2 flex items-center justify-center text-white text-xs font-bold">
                {initials}
              </div>
            )}
          ...
          <button
            ...
            className="w-full bg-red-50 text-red-600 border border-red-200 rounded-lg py-2 text-sm font-semibold hover:bg-red-100 transition-colors"
          >
            🚪 Logout
          </button>
        </div>
          ...
          <>
            ...
            >
              ...
              <nav className="flex-1 p-3 space-y-1">
                {NAV.map(n => <NavLink key={n.path} n={n} onClick={() => setSidebarOpen(false)} />)}
              </nav>
              <div className="p-3 border-t border-agri-7">
                <button onClick={handleLogout} className="w-full bg-red-50 text-red-600 border border-red-200 rounded-lg py-2.5 text-sm font-semibold">
                  🚪 Logout
                </button>
              </div>
            </motion.aside>
        ...
        <header className="bg-white border-b border-agri-7 px-4 md:px-6 h-14 flex items-center justify-between shrink-0">
          ...
          <div className="flex items-center gap-2">
            ...
            <div
              ...
              className="w-9 h-9 rounded-full overflow-hidden bg-gradient-to-br from-agri-4 to-agri-2 flex items-center justify-center text-white text-xs font-bold cursor-pointer"
            >
              {user?.picture ? (
      ...
      <nav className="md:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-agri-7 flex z-30">
        {NAV.map(n => (
          <Link
            key={n.path}
            to={n.path}
